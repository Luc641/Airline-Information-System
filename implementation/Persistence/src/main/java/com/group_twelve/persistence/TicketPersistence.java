...
public class TicketPersistence implements Persistence<Ticket>{
    ...
    public List<Ticket> getTicketsByFlightId(int ID){
        ...
        try{
            ...
            while(tickets.next()){
                list.add(creator.apply(new Integer[]{tickets.getInt(1),tickets.getInt(2),tickets.getInt(3),tickets.getInt(4)}));
            }
    ...
    public boolean save(Ticket ticket) {
        String query = String.format("INSERT INTO ticket (seatnr, flightid, options) VALUES (%s,%s,%s)", ticket.getSeatNr(), ticket.getFlightID(), ticket.getOptionID());
        try{
            db.query(query);
            return true;
        }catch(Exception e){
